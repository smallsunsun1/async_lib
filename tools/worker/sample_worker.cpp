...
class RpcWorkerImpl : public RpcWork::Service {
 public:
  grpc::Status RemoteCall(grpc::ServerContext* context, const Request* request,
                          Response* response) override {
    (void)context;
    response->set_type(request->type());
    return grpc::Status::OK;
  }
  grpc::Status RemoteStreamCall(
...
class RpcWorkAsyncUnaryServer {
 public:
  ~RpcWorkAsyncUnaryServer() {
    server_->Shutdown();
 ...
 private:
  struct CallData {
    ...
    void Proceed() {
      if (status == CREATE) {
        status = PROCESS;
        service->RequestRemoteCall(&ctx_, &request, &responder_, queue, queue,
                                   (void*)this);
...
class RpcWorkAsyncStreamServer {
 public:
  RpcWorkAsyncStreamServer(async::HostContext* context, uint32_t num_threads)
      : run_context_(context) {
    threads_.resize(num_threads);
    server_queues_.resize(num_threads);
    shutdown_state_.resize(num_threads);
    compute_func_ = [this](Request* req, Response* res) -> grpc::Status {
      return DoRunFunc(req, res);
    };
  }
  ~RpcWorkAsyncStreamServer() {
  ...
  void Run(const std::string& server_address) {
    ...
    for (uint32_t connection_num = 0; connection_num < kNumCalls;
         ++connection_num) {
      for (size_t i = 0; i < server_queues_.size(); ++i) {
        async_call_impl_queue_.emplace_back(
            new AsyncCallDataImpl(service_.get(), server_queues_[i].get(),
                                  run_context_, compute_func_));
      }
    }
    for (size_t i = 0; i < threads_.size(); ++i) {
      shutdown_state_[i] = std::make_unique<PerThreadShutDownState>();
      threads_[i] =
          std::make_unique<std::thread>([this, i]() { HandleRpcs(i); });
    }
    server_->Wait();
  }
 ...
 private:
  class AsyncCallDataImpl : public RpcServerContext {
   public:
    AsyncCallDataImpl(
        RpcWork::AsyncService* service, grpc::ServerCompletionQueue* queue,
      ...
      stream_ =
          std::make_unique<grpc::ServerAsyncReaderWriter<Response, Request>>(
              server_context_.get());
      service_->RequestRemoteStreamCall(server_context_.get(), stream_.get(),
                                        complete_queue_, complete_queue_,
...
int main(int argc, char* argv[]) {
  ...
  if (absl::GetFlag(FLAGS_use_health_check)) {
    grpc::EnableDefaultHealthCheckService(true);
    grpc::ServerBuilder builder;
    builder.AddListeningPort(FLAGS_server_address.CurrentValue(),
                             grpc::InsecureServerCredentials());
    std::unique_ptr<grpc::Server> server(builder.BuildAndStart());
    grpc::HealthCheckServiceInterface* service =
        server->GetHealthCheckService();
    service->SetServingStatus(grpc::kHealthCheckServiceInterfaceArg, true);
  }
