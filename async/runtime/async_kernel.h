...
struct AsyncValueInfo {
  ...
  AsyncValueInfo(AsyncValueInfo&& value) : mUserCount(value.mUserCount) { mValue.store(value.mValue.load()); }
  explicit AsyncValueInfo(unsigned userCount) : mUserCount(userCount) {}
};
struct KernelResInfo {
  std::vector<std::vector<unsigned>> mResultTable;  // 对应于第几个kernel+第几个输出结果
};
struct KernelInfo {
  KernelInfo(KernelInfo&& value) { mArgumentsNotReady.store(value.mArgumentsNotReady.load()); }
  std::atomic<int> mArgumentsNotReady;
  KernelInfo() = default;
  KernelInfo(unsigned numOperands) : mArgumentsNotReady(std::max(static_cast<unsigned>(1), numOperands)) {}
};
struct FunctionInfo {
  std::vector<AsyncValueInfo> mAsyncValueInfos;  // 存储整个Function调用过程中的对应AsyncValue和它的userCount
