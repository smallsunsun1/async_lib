...
class AsyncKernelFrame {
 public:
  ...
  AsyncValue *GetArgAt(int index) const {
    assert(index < GetNumArguments() &&
           "Get Arg Index Must Smaller Than Num Arguments!");
    return mAsyncValues[index];
  }
  absl::Span<AsyncValue *const> GetArguments() const {
    return GetAsyncValues(0, mNumArguments);
  }
  ...
  absl::Span<AsyncValue *const> GetResults() const {
    return GetAsyncValues(mNumArguments, mNumResults);
  }
  absl::Span<AsyncValue *> GetResults() {
  ...
  void PropogateError(std::string_view errorMsg) {
    bool setError = false;
    RCReference<ErrorAsyncValue> errorValue =
        mpContext->MakeErrorAsyncValueRef(errorMsg);
 ...
 protected:
  absl::Span<AsyncValue *const> GetAsyncValues(size_t from,
                                               size_t length) const {
    assert((from + length) <=
               static_cast<size_t>(mNumArguments + mNumResults) &&
           "from + length Must Less Than Total Asyncvalue Size");
    if (length == 0) return {};
...
class CommonAsyncKernelFrame : public AsyncKernelFrame {
 public:
  ...
  void AddArg(AsyncValue *argument) {
    assert(mNumResults == -1 &&
           "Must Call AddArg Before Calling SetNumResults");
    mAsyncValues.push_back(argument);
    mNumArguments++;
  }
  void SetNumResults(int size) {
    assert(static_cast<size_t>(mNumArguments) == mAsyncValues.size() &&
           "NumArguments Must Equal to AsyncValues size");
