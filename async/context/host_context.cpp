...
HostContext::HostContext(std::function<void(const DecodedDiagnostic&)> diag_handler, std::unique_ptr<HostAllocator> allocator, std::unique_ptr<ConcurrentWorkQueue> work_queue)
  ...
  assert(instance_index() < HostContextPtr::kDummyIndex && "Created too many HostContext instances");
...
RCReference<IndirectAsyncValue> HostContext::MakeIndirectAsyncValue() { return TakeRef(Construct<IndirectAsyncValue>(mInstancePtr)); }
...
// diagnostic.
RCReference<ErrorAsyncValue> HostContext::MakeErrorAsyncValueRef(DecodedDiagnostic&& diagnostic) {
  // Create an AsyncValue for this error condition.
  auto* error_value = Construct<ErrorAsyncValue>(mInstancePtr, std::move(diagnostic));
...
void HostContext::CancelExecution(std::string_view msg) {
  ...
  if (!mCancelValue.compare_exchange_strong(expected_value, error_value, std::memory_order_release, std::memory_order_relaxed)) {
    error_value->DropRef();
  }
}
...
void* HostContext::AllocateBytes(size_t size, size_t alignment) { return mAllocator->AllocateBytes(size, alignment); }

// Deallocate the specified pointer, that had the specified size.
void HostContext::DeallocateBytes(void* ptr, size_t size) { mAllocator->DeallocateBytes(ptr, size); }
...
void HostContext::Await(absl::Span<const RCReference<AsyncValue>> values) { mWorkQueue->Await(values); }

// Add some work to the workqueue managed by this CPU device.
void HostContext::EnqueueWork(unique_function<void()> work) { mWorkQueue->AddTask(TaskFunction(std::move(work))); }
...
int HostContext::GetNumWorkerThreads() const { return mWorkQueue->GetParallelismLevel(); }
...
void HostContext::RunWhenReady(absl::Span<AsyncValue* const> values, unique_function<void()> callee) {
  ...
  if (unavailable_values.size() == 1) {
    unavailable_values[0]->AndThen([callee = std::move(callee)]() mutable { callee(); });
    return;
  }
  ...
  auto* data = new CounterAndCallee{{unavailable_values.size()}, std::move(callee)};
...
void HostContext::RunWhenReady(absl::Span<const RCReference<AsyncValue>> values, unique_function<void()> callee) {
  ...
  RunWhenReady(absl::MakeConstSpan(values_ptr.data(), values_ptr.size()), std::move(callee));
}
...
class HostContext::SharedContextManager {
 ...
 private:
  ...
  std::array<std::pair<std::unique_ptr<SharedContext>, std::once_flag>, 256> shared_context_instances_{};
};

SharedContext& HostContext::GetOrCreateSharedContext(int shared_context_id, SharedContextFactory factory) { return mSharedCtxMgr->GetOrCreateSharedContext(shared_context_id, factory); }

bool HostContext::IsInWorkerThread() const { return mWorkQueue->IsInWorkerThread(); }

SharedContext::~SharedContext() {}

std::unique_ptr<HostContext> CreateSimpleHostContext() {
  return std::make_unique<HostContext>([](const DecodedDiagnostic& in) { std::cout << in.message << std::endl; }, CreateMallocAllocator(),
                                       CreateMultiThreadedWorkQueue(std::thread::hardware_concurrency(), 2 * std::thread::hardware_concurrency()));
}
