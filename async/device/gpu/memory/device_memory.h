...
class DeviceMemory {
 public:
  class PointDeviceTypePair {
   public:
    ...
    bool operator==(const PointDeviceTypePair &other) const {
      return mpData == other.mpData && mType == other.mType;
    }
    bool operator!=(const PointDeviceTypePair &other) const {
  ...
  template <typename U>
  using implicit_convertible_t =
      typename std::enable_if_t<std::is_constructible<U *, T *>::value, bool>;
  template <typename U>
  using explicit_convertible_t = typename std::enable_if_t<
      !std::is_constructible<U *, T *>::value &&
          (std::is_constructible<T *, U *>::value ||
           std::is_same<typename std::remove_cv_t<U>, void>::value),
      bool>::type;
  template <typename U>
  ...
  DeviceMemory(const DeviceMemory<U> &other)
      : mPair(other.Pointer(), other.GetDeviceType()) {}
  template <typename U, explicit_convertible_t<U> = false>
  explicit DeviceMemory(const DeviceMemory<U> &other)
      : mPair(static_cast<T *>(other.Pointer()), other.GetDeviceType()) {}
  template <typename U>
  DeviceMemory &operator=(const DeviceMemory<U> &other) {
    mPair.setPointerAndInt(static_cast<T *>(other.Pointer()),
                           other.GetDeviceType());
    return *this;
  }
  ...
  DeviceMemory operator+(std::ptrdiff_t offset) const {
    return DeviceMemory(Pointer() + offset, GetDeviceType());
  }
  DeviceMemory operator-(std::ptrdiff_t offset) const {
    return DeviceMemory(Pointer() - offset, GetDeviceType());
  }
 ...
 private:
  ...
  typename std::conditional<
      PointerLikeTypeTraits<T *>::NumLowBitsAvailable >= 2,
      PointerIntPair<T *, 2, DeviceType>, PointDeviceTypePair>::type
      mPair;  //通常指针align大小为3bit，这里小于2bit可以压缩到一个PointIntPair
  friend std::ostream &operator<<(std::ostream &os,
                                  const DeviceMemory &Pointer) {
    return os << Pointer.data() << "\n";
  }
};
