 ...
 public:
  ...
  ~TimeInfoUtility() {
    for (const auto& timeInfo : mActivities) {
      auto startUs = std::chrono::duration_cast<std::chrono::microseconds>(timeInfo.start - kProcessStart);
  ...
  void PopScope() {
    ...
    mActivities.push_back({std::move(std::get<0>(mStack.back())), std::get<1>(mStack.back()), Now()});
    mStack.pop_back();
  }
