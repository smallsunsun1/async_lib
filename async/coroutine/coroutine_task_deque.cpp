...
absl::optional<ScheduleOperation*> CoroutineTaskDeque::PushFront(ScheduleOperation* task) {
...
absl::optional<ScheduleOperation*> CoroutineTaskDeque::PopFront() {
  unsigned front = mFront.load(std::memory_order_relaxed);
...
absl::optional<ScheduleOperation*> CoroutineTaskDeque::PopBack() {
  ...
  return absl::optional<ScheduleOperation*>(std::move(task));
}
