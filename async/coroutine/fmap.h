...
class FmapAwaiter {
 ...
 public:
  FmapAwaiter(Func &&func, Awaitable &&awaitable) noexcept(
      std::is_nothrow_move_constructible_v<awaiter_t> &&noexcept(
          internal::get_awaiter(static_cast<Awaitable &&>(awaitable))))
      : mFunc(static_cast<Func &&>(func)),
        m_awaiter(internal::get_awaiter(static_cast<Awaitable &&>(awaitable))) {
  }
  ...
  decltype(auto) await_suspend(std::coroutine_handle<Promise> coro) noexcept(
      noexcept(static_cast<awaiter_t &&>(m_awaiter).await_suspend(
          std::move(coro)))) {
  ...
  decltype(auto) await_resume() noexcept(
      noexcept(std::invoke(static_cast<Func &&>(mFunc)))) {
  ...
  decltype(auto) await_resume() noexcept(noexcept(
      std::invoke(static_cast<Func &&>(mFunc),
                  static_cast<awaiter_t &&>(m_awaiter).await_resume()))) {
    return std::invoke(static_cast<Func &&>(mFunc),
                       static_cast<awaiter_t &&>(m_awaiter).await_resume());
  }
...
class FmapAwaitable {
 ...
 public:
  template <
      typename FuncArg, typename AwaitableArg,
      std::enable_if_t<std::is_constructible_v<Func, FuncArg &&> &&
                           std::is_constructible_v<Awaitable, AwaitableArg &&>,
                       int> = 0>
  explicit FmapAwaitable(FuncArg &&func, AwaitableArg &&awaitable) noexcept(
      std::is_nothrow_constructible_v<Func, FuncArg &&>
          &&std::is_nothrow_constructible_v<Awaitable, AwaitableArg &&>)
      : mFunc(static_cast<FuncArg &&>(func)),
        mAwaitable(static_cast<AwaitableArg &&>(awaitable)) {}

  auto operator co_await() const & {
    return FmapAwaiter<const Func &, const Awaitable &>(mFunc, mAwaitable);
  }

  auto operator co_await() & {
    return FmapAwaiter<Func &, Awaitable &>(mFunc, mAwaitable);
  }

  auto operator co_await() && {
    return FmapAwaiter<Func &&, Awaitable &&>(
        static_cast<Func &&>(mFunc), static_cast<Awaitable &&>(mAwaitable));
  }
...
struct FmapTransform {
  explicit FmapTransform(Func &&f) noexcept(
...
template <typename Func, typename Awaitable,
          std::enable_if_t<internal::is_awaitable_v<Awaitable>, int> = 0>
auto Fmap(Func &&func, Awaitable &&awaitable) {
  return internal::FmapAwaitable<
