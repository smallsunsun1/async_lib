...
using WorkQueueFactoryMap =
    absl::flat_hash_map<absl::string_view, WorkQueueFactory>;
...
void RegisterWorkQueueFactory(absl::string_view name,
                              WorkQueueFactory factory) {
...
std::unique_ptr<ConcurrentWorkQueue> CreateWorkQueue(absl::string_view config) {
  ...
  absl::string_view name =
      colon == absl::string_view::npos ? config : config.substr(0, colon);
  absl::string_view args =
      colon == absl::string_view::npos ? "" : config.substr(colon + 1);
