...
namespace detail {
template <typename T>
using enable_if_trivial =
    std::enable_if_t<std::is_trivially_move_constructible<T>::value &&
                     std::is_trivially_destructible<T>::value>;
template <typename callableT, typename thisT>
using enable_unless_same_type =
    std::enable_if_t<!std::is_same<std::decay_t<callableT>, thisT>::value>;
...
class UniqueFunctionBase {
 protected:
  ...
  template <typename T>
  using adjusted_param_t = typename std::conditional<
      !std::is_reference<T>::value &&
          std::is_trivially_copy_constructible<T>::value &&
          std::is_trivially_move_constructible<T>::value &&
          IsSizeLessThanThresholdT<T>::value,
      T, T &>::type;
  using call_ptr_t = ReturnT (*)(void *callableAddr,
                                 adjusted_param_t<ParamsTs>... Params);
  ...
  using call_back_pointer_union_t =
  ...
  union StorageUnionT {
    ...
    static_assert(
        sizeof(OutOfLineStorageT) <= kInlineStorageSize,
        "should always use all of the out-of-line storage for inline storage!");
    mutable
        typename std::aligned_storage<kInlineStorageSize, alignof(void *)>::type
            inlineStorage;
  } storageUnion;
  ...
  call_ptr_t getTrivialCallBack() const {
  ...
  UniqueFunctionBase(CallableT callable, CalledAs<CalledAsT>) {
    ...
    if (sizeof(CallableT) > kInlineStorageSize ||
    ...
    callbackAndInlineFlag.setPointerAndInt(
        &CallbacksHolder<CallableT, CalledAsT>::callbacks, isInlineStorageV);
  }
  ~UniqueFunctionBase() {
    ...
    if (!isTrivialCallBack()) {
      getNonTrivialCallbacks()->destroyPtr(
          isInlineStorageV ? getInlineStorage() : getOutOfLineStorage());
    }
    if (!isInlineStorage()) {
...
typename UniqueFunctionBase<R, P...>::TrivialCallback
    UniqueFunctionBase<R, P...>::CallbacksHolder<
        CallableT, CalledAsT, enable_if_trivial<CallableT>>::callbacks{
        &CallImpl<CalledAsT>};
...
class unique_function<R(Ts...)> : public detail::UniqueFunctionBase<R, Ts...> {
 ...
 public:
  ...
  unique_function(
      CallableT callable,
      detail::enable_unless_same_type<CallableT, unique_function> * = nullptr,
      detail::enable_if_callable<CallableT, R, Ts...> * = nullptr)
      : Base(std::forward<CallableT>(callable),
             typename Base::template CalledAs<CallableT>{}) {}
  R operator()(Ts... params) {
    return this->getCallPtr()(this->getCalleePtr(), params...);
  }
};
template <typename R, typename... Ts>
class unique_function<R(Ts...) const>
    : public detail::UniqueFunctionBase<R, Ts...> {
  using Base = detail::UniqueFunctionBase<R, Ts...>;
 ...
 public:
  ...
  unique_function(
      CallableT callable,
      detail::enable_unless_same_type<CallableT, unique_function> * = nullptr,
      detail::enable_if_callable<const CallableT, R, Ts...> * = nullptr)
      : Base(std::forward<CallableT>(callable),
